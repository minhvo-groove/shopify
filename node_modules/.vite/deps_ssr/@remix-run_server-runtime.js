  ...
  return async ({
    ...
    let results = await Promise.all(matches.map(async (match) => {
      let result = await match.resolve(async (handler) => {
        let data3 = loadRouteIds && !loadRouteIds.includes(match.route.id) ? null : await handler();
        return {
          type: "data",
          result: data3
        };
...
var defineAction = (action) => action;
    ...
    for (let path of url.searchParams.getAll("p")) {
      ...
      if (matches) {
        for (let match of matches) {
          ...
          data3.patches[routeId] = build.assets.routes[routeId];
        }
      } else {
        data3.notFoundPaths.push(path);
      }
    }
    return json3(data3, {
      headers: {
