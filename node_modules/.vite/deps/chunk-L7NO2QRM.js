...
function createRouter(init) {
  ...
  if (!initialMatches) {
    ...
    if (future.v7_partialHydration) {
      let fogOfWar2 = checkFogOfWar(null, dataRoutes, init.history.location.pathname);
      if (fogOfWar2.active && fogOfWar2.matches) {
        initialMatches = fogOfWar2.matches;
      }
    }
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    let fogOfWar2 = checkFogOfWar(matches, routesToUse, location.pathname);
    if (fogOfWar2.active && fogOfWar2.matches) {
      matches = fogOfWar2.matches;
    }
    if (!matches) {
      let {
        error,
    ...
    if (opts && opts.pendingError) {
      ...
      let actionResult = await handleAction(request, location, opts.submission, matches, fogOfWar2.active, {
    ...
    let {
      ...
      loaderData,
      errors
    } = await handleLoaders(request, location, matches, fogOfWar2.active, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, opts && opts.initialHydration === true, flushSync, pendingActionResult);
    if (shortCircuited) {
      return;
    }
    pendingNavigationController = null;
  ...
  function fetch2(key, routeId, href, opts) {
    ...
    let fogOfWar2 = checkFogOfWar(matches, routesToUse, normalizedPath);
    if (fogOfWar2.active && fogOfWar2.matches) {
      matches = fogOfWar2.matches;
    }
    if (!matches) {
    ...
    if (submission && isMutationMethod(submission.formMethod)) {
      handleFetcherAction(key, routeId, path, match, matches, fogOfWar2.active, flushSync, submission);
      return;
    }
    ...
    handleFetcherLoader(key, routeId, path, match, matches, fogOfWar2.active, flushSync, submission);
  }
  ...
  return Promise.all(matches.map(async (m) => {
    ...
    let result = await m.resolve(async (handler) => {
      let result2 = await handler(async () => {
        ...
        actionStatus = status;
        return unwrapSingleFetchResult(data3, m.route.id);
      });
      return {
        type: "data",
        result: result2
...
function useFogOFWarDiscovery(router2, manifest, routeModules, future, isSpaMode) {
  React7.useEffect(() => {
    ...
    async function fetchPatches() {
      ...
      try {
        await fetchAndApplyManifestPatches(lazyPaths, fogOfWar, manifest, routeModules, future, isSpaMode, router2.basename, router2.patchRoutes);
      } catch (e) {
...
function getFogOfWarPaths(fogOfWar2, router2) {
  let {
    knownGoodPaths,
    known404Paths,
    nextPaths
...
async function fetchAndApplyManifestPatches(paths, _fogOfWar, manifest, routeModules, future, isSpaMode, basename, patchRoutes) {
  ...
  let patches = Object.values(data3.patches).reduce((acc, route) => !knownRoutes.has(route.id) ? Object.assign(acc, {
    [route.id]: route
  }) : acc, {});
  Object.assign(manifest.routes, patches);
  data3.notFoundPaths.forEach((p) => known404Paths.add(p));
...
function PrefetchPageLinksImpl({
  ...
  let dataHrefs = React8.useMemo(() => getDataLinkHrefs(page, newMatchesForData, manifest), [newMatchesForData, page, manifest]);
  ...
  if (!future.unstable_singleFetch) {
    linksToRender = dataHrefs.map((href) => React8.createElement("link", _extends4({
      key: href,
      rel: "prefetch",
      as: "fetch",
      href
    ...
    if (url.searchParams.get("_routes") !== "") {
      linksToRender = React8.createElement("link", _extends4({
  ...
  return React8.createElement(React8.Fragment, null, linksToRender, moduleHrefs.map((href) => React8.createElement("link", _extends4({
    key: href,
    rel: "modulepreload",
    href
...
function RemixBrowser(_props) {
  ...
  return (
    ...
    }))), window.__remixContext.future.unstable_singleFetch ? React9.createElement(React9.Fragment, null) : null)
  );
}
...
function RemixServer({
  ...
  return React12.createElement(React12.Fragment, null, React12.createElement(RemixContext.Provider, {
    ...
    hydrate: false
  }))), context.future.unstable_singleFetch && context.serverHandoffStream ? React12.createElement(React12.Suspense, null, React12.createElement(StreamTransfer, {
    context,
    identifier: 0,
...
@remix-run/react/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-L7NO2QRM.js.map
