...
async function ensureValidNodeAlgorithm(algorithm) {
  if (typeof algorithm === "object" && (algorithm.name === "X448" || algorithm.name === "Ed448")) {
    throw new DOMException("Unrecognized name.", "NotSupportedError");
  }
  if (typeof algorithm === "object" && algorithm.name === "Ed25519" && !await usesModernEd25519) {
  ...
  dispatch(options, handler) {
    ...
    if (Array.isArray(headers)) {
      ...
      for (const key in headers) {
        if (this.removeHeaders.includes(key.toLowerCase()))
          delete headers[key];
      }
    }
...
async function waitUntilAll(promises) {
  let len = 0;
  let last = [];
  while (len !== promises.length) {
    len = promises.length;
    last = await Promise.all(promises);
  }
  return last;
}
...
Make sure you're ${suggestion}.`);
      ...
      const waitUntil2 = waitUntilAll(event[kWaitUntil2]);
...
Make sure you're ${suggestion}.`);
    ...
    const waitUntil = waitUntilAll(event[kWaitUntil2]);
  ...
  async [kDispatchScheduled](scheduledTime, cron) {
    ...
    super.dispatchEvent(event);
    return waitUntilAll(event[kWaitUntil2]);
  }
...
__decorateClass([
  (0, import_shared9.Option)({
    ...
    fromWrangler: ({ build, miniflare }) => {
      ...
      if (build?.watch_dir) {
        watchPaths.push(...Array.isArray(build.watch_dir) ? build.watch_dir : [build.watch_dir]);
      }
  ...
  async setup() {
    ...
    if (this.script !== void 0) {
      return {
        ...
        additionalModules,
        script: {
          filePath: this.scriptPath ?? import_shared10.STRING_SCRIPT_PATH,
          code: this.script
        }
      };
    }
